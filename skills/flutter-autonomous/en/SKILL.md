...
- **Device** ← taken at runtime from `mobilecli devices` after the §2 bootstrap; **never write a device id into any file**
...
One shot: `bash scripts/bootstrap.sh` (cross-platform mac/Linux, Android+iOS, idempotent and re-entrant: each item is **detect → install if missing → re-verify with an independent command → skip if already present**). Item by item:
...
**Flutter locator priority (most to least robust)**: Patrol `Key` (most robust for regression) > exact `Semantics` label > role/`button:true` flag > label substring/regex > plain text > blind coordinates (last resort).

Key points: take coordinates from the `dump ui` rect center, **never guess**; `screenshot` + Read after each step; if you tap wrong, `io button BACK`; Flutter's **custom-painted numeric keypads and custom gesture widgets are not system text fields** — `io text` won't reach them → tap each key's coordinates with `io tap`; **a WDA-synthesized `io longpress` may not be recognized on the Flutter side** (e.g. a long-press `GestureDetector` on an AppBar title) — don't fight it: apply the same "can't list it = go fix the code" principle below and replace the hidden gesture with a tappable widget that has `Semantics` (which also makes manual testing better); if a widget can't be listed = it doesn't expose Semantics → **go fix the code** (below), don't settle for blind coordinates. Deeplink shortcut: `mobilecli device url <deeplink>` jumps straight to a page, skipping step-by-step navigation.

**The "silent failure" list — sent ≠ took effect.** The backend only guarantees "the event was dispatched", not "Flutter received and recognized it", and **the exit code is 0 either way when it fails**: `io swipe` (Flutter's scroll gestures are sensitive to a synthesized event's duration/step, and the typical symptom is a screen that doesn't move at all), a synthesized `io longpress` (above), and **tapping the blank area of a container that wraps your target** (after Semantics merging an ancestor's label naturally contains the substring, and in `dump ui` it comes *before* the leaf — pick the smallest rect by area). These commands **must be confirmed with outside evidence**: after a swipe, `dump ui` again and compare an anchor element's `rect.y` (or diff before/after screenshots); if nothing moved, retry once with a longer distance / slower duration, and **if it doesn't move twice, change route** (deeplink straight to the page, or let the Dart side scroll itself via Patrol's `scrollTo`) rather than burning rounds on the same command. `io tap` is more reliable but the same rule applies — screenshot or dump afterwards to verify where it landed. **This is the most common on-device form of the Never anti-pattern "treating 'I ran the action' as 'the result happened'"**, and the usual real cause behind a "tapped it, nothing happened" conclusion.

---

## Code contract: every interactive/assertable widget gets a Key + Semantics

The two paths each consume a different thing — add both, and the widget becomes "testable by construction": `Key` for Patrol (named `<feature>_<widget_type>`, lowercase with underscores); `Semantics(label:)` for element-driven interaction. Standard `Text`/`ElevatedButton` text carries a label already; **custom gesture widgets (`Touchable`/`GestureDetector`/`InkWell`) can't be listed by default — always wrap them explicitly in `Semantics(label + button:true)`**.

```dart
ElevatedButton(key: const Key('submit_btn'), onPressed: _submit, child: const Text('Submit'))

Semantics(label: 'Slide to buy', button: true,      // custom gesture: without Semantics it won't show in dump
...
**How many platforms to run is decided by the nature of the change — don't default to both, don't default to one.** First check how many the project actually ships (the `android/` and `ios/` directories, which artifacts CI builds) — **a single-platform project runs that platform only**; don't chase "parity" onto a platform this project doesn't ship:
...
> **Don't use a signal when you need to know whether the reload succeeded**: `kill -USR1` is fire-and-forget — you're left grepping output and guessing. When you must confirm "did this reload actually take", use `app.restart` over `flutter run --machine` — it **returns `{"code":0,"message":"Reloaded N libraries"}`**, and `code!=0` is the assertion itself. See `references/vm-service.md` §2.4. For casual day-to-day reloads, the signal is still less hassle.
...
```
**Before announcing "tested/stopped", re-check the real state with an independent command** — don't treat "I ran kill" as "the app is closed". **Device system state you changed during testing must be restored and re-checked too** — if you cut the network, you must verify it's back (`svc wifi enable` hangs on some models; the reliable recovery path is in `references/android.md` §10).
...
2. `dump ui` to inspect before interacting; locate by Key/label; take coordinates from the rect center (with several matches, pick the **smallest by area** — the big ones are usually the container wrapping it); `io swipe` and synthesized long-presses **can fail silently** — the exit code is 0 either way, so dump again and compare an anchor rect, or screenshot, to confirm; if it doesn't move twice, change route.
3. Double-tag interactive/assertable widgets with `Key` + `Semantics`; if `dump ui` can't list it, go fix the code, and use `meetsGuideline(labeledTapTargetGuideline)` so it gets caught automatically from then on.
...
6. Self-expand the acceptance criteria into 3–8 assertable items and tag each with its layer; **before each item, terminate→launch→dump to reset to a known starting point**; decide how many platforms by the nature of the change, never write single-platform green as both-platform green, and **go straight to a real device for what a simulator can't give you** (screenshot protection/performance/biometrics/push/sensors) (§The full autonomous loop).
...
8. Change code via `--pid-file` + `USR1`/`USR2`, not cold starts; **if you changed a codegen input (`.arb`/freezed/drift), run the generator before USR2** instead of judging "it didn't take" from stale output; **to judge whether a reload succeeded, use `app.restart` over `--machine`**, don't guess from grepped output. When waiting for a build, provide all three exits — "it's up / it died / it's stuck" — never wait on the success signal alone.
9. Two teardown steps to close the app, then **re-check to confirm**; **no verification, no completion claim**.
...
**Never — anti-patterns**: blind-tapping when Semantics exists / hardcoding historical coordinates / treating `kill flutter run` as closing the app / editing tests to dodge assertions / treating "I performed the action" as "the result was achieved" (**concrete form: taking `io swipe`'s exit code 0 as a successful swipe**) / verifying on a simulator what a simulator cannot simulate (screenshot protection/performance/biometrics) and writing it up as passed / **passing "the offline tests are all green" off as "the UI is verified"** (the #1 anti-pattern: not going on-device when you should have, and a report with not a single screenshot) / **spending real-device time locating a pure-logic bug** (the offline layer pinpoints the line in seconds) / **eyeballing full-screen screenshots for repetitive visual regression** (lock down what should be a golden).

> Of these, **"passing offline-green off as UI-verified" is the one to guard hardest**: the rest are efficiency problems, that one is **claiming completion without doing the work** — this skill's whole value is in that screenshot and that real run.
