...
**探一次就够,别每步重探**:这些值在一个项目里是稳定的,本次会话探到就**记住复用**——同一次运行里反复 grep `build.gradle` 是纯浪费。运行环境若有跨会话记忆,把**稳定事实**存进去下次直接用:包名与两端 id、入口与 dart-defines、这个项目实际发几端、Patrol 用例放哪、冷启构建大概多久。**没有记忆机制也无所谓**,重探一次就几秒——别为了省这几秒去要求用户配置什么。
...
**「静默失败」清单——发出去 ≠ 生效**。底座只保证「事件发出去了」,不保证「Flutter 收下并识别了」,而**失败时退出码照样是 0**:`io swipe`(Flutter 滚动手势对合成事件的时长/步进敏感,典型表现是屏幕纹丝不动)、合成 `io longpress`(上条)、以及**点在了包住目标的容器空白处**(Semantics 合并后祖先节点的 label 天然含子串,`dump ui` 里它还排在叶子前面——按 rect 面积挑最小的那个)。这类命令**必须拿外部证据确认**:滑动后重新 `dump ui` 比对锚点元素的 `rect.y` 有没有变(或前后截图对比);没变就加大位移/放慢时长重试一次,**两次不动就换路径**(deeplink 直达、或 Patrol 的 `scrollTo` 让 Dart 侧自己滚),别在同一条命令上刷轮次。`io tap` 相对可靠但同理——点完必须截图或 dump 核验落点。**这是 Never 反模式「把执行了操作当达到了结果」在设备层最常见的形态**,也是「点了没反应」这类判断最常见的真因。
...
**跑几端由改动性质决定,别默认双端也别默认单端**。先看项目实际发几端(`android/`、`ios/` 目录、CI 里在打哪些包)——**只发一端的项目就只跑那端**,别为了"对等"去跑一个这个项目根本不发的平台:
...
- **但有一类东西模拟器给不了,它的绿是假绿——直接上真机,别先在模拟器上验**:防截屏与安全层(`FLAG_SECURE`/iOS secure layer,模拟器照样截得到,看起来像"没生效")、真实性能与掉帧、生物识别、推送、相机与传感器、完整性/证明类 SDK、真实网络条件与弱网。**判据**:被测的东西依赖的是「真设备才有的能力或约束」,就跳过模拟器这一档。
- **想跑但跑不了**(Linux 无 iOS 工具链、手上没那台真机):跑得了的跑透,跑不了的标「未验证:<平台>,原因:<无工具链/无设备>」——**不许把单端绿写成双端绿**。

**每条验收条目开跑前,先把 App 复位到已知起点**。上一条留下的页面状态会变成下一条的错误起点——停在某个 bottom sheet 里、还开着筛选面板、卡在半截表单——**越往后越歪,而报告里完全看不出来**(每一步的截图都"有内容",只是验错了页面);无人值守连跑时这种漂移会被逐条放大。复位就三步:`apps terminate` → `apps launch` → `dump ui` 确认落在预期起点,几秒的成本换掉整条验收作废的风险。**别把「上一条跑完了」当「现在在首页」**——和收尾一样,状态要用独立命令回查,不靠推断。
...
> 三个出口分别对应「起来了 / 死了 / 还没好」。**只有「进程已退出」是失败判据**;到点未就绪**不是**——冷仓首次 Gradle/CocoaPods、大工程、慢网、CI 容器里构建半小时都正常。到点时看 `<LOG_FILE>` 尾部:**进程还活着、日志还在长 = 还在编,再等一轮**(时限翻倍,别改成无限);日志停在某条错误上才是真卡住。**别把闹钟响了当构建失败去 `flutter clean` 重来**——那是把一次慢构建变成两次。
>
...
2. 交互前先 `dump ui` 检视,按 Key/label 定位,坐标取 rect 中心(多个匹配挑**面积最小**的,大的多半是包住它的容器);`io swipe`/合成长按属**可能静默失败**——失败时退出码照样是 0,发完必须重新 dump 比对锚点 rect 或截图确认,两次不动就换路径。
...
6. 验收标准先自展开成 3~8 条可断言条目并逐条标层;**每条开跑前 terminate→launch→dump 复位到已知起点**;跑几端按改动性质定,单端绿不写成双端绿,**模拟器给不了的能力(防截屏/性能/生物识别/推送/传感器)直接上真机**(§自主开发完整循环)。
7. 找不到控件 / 布局歪 / 疑似报错,**先查 VM Service**(widget 树带源码行号、render 树给真实 constraints),别一上来就截图肉眼找。
8. 改代码走 `--pid-file` + `USR1`/`USR2`,不冷启;**改了 codegen 输入(`.arb`/freezed/drift)先跑生成器再 USR2**,别拿旧产物判"没生效";**要判重载成败用 `--machine` 的 `app.restart`**,别 grep 输出猜。等构建必须给全「起来了/死了/超时」三个出口,别只等成功信号。
9. 收尾两步关 App 并**回查确认**;**不验证不报完成**。
...
**Never — 反模式**:有 Semantics 还盲点 / 写死历史坐标 / 把 `kill flutter run` 当关 App / 改测试绕过断言 / 把"执行了操作"当"达到了结果"(**具体形态:拿 `io swipe` 的退出码 0 当滑动成功**) / 在模拟器上验它根本模拟不了的能力(防截屏/性能/生物识别)还写成通过 / **拿"离线测试全绿"当"UI 验过了"**(头号反模式:该上设备的没上,报告里没有一张截图) / **纯逻辑 bug 拿真机时间去定位**(离线层秒级能定位到行) / **靠肉眼盯整屏截图做重复性视觉回归**(该固化成 golden 的固化掉)。

> 这几条里**「拿离线绿冒充 UI 验过」最该防**:其余是效率问题,它是**没干活却报了完成**——本 skill 的价值就在那张截图和那次真跑上。
