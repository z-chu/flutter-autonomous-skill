...
> ⚠️ **先摆正位置,别本末倒置**:本 skill 的主场是**把 App 跑到设备上看 UI**(见 [SKILL.md](../SKILL.md) §0——纯写单测根本不该动用本 skill)。这三层的作用是**上设备之前先筛掉不值得占用设备时间的东西**,让设备时间集中在只有设备能验的事上;**它们是给设备层让路的,不是设备层的替代品**。
>
> - **值得往下沉的**:纯逻辑 bug(解析错、算错数、空指针)拿真机时间去定位是浪费,离线层秒级定位到行;反复跑的静态视觉回归可以用 golden 锁住,不必每次人工重看同一屏。
> - **绝不能往下沉的**:**离线全绿 ≠ UI 没问题**。widget test 跑在无头环境,不经真实渲染管线、没有真实字体度量、没有平台通道、没有真机时序。它能证「逻辑上该显示 X」,**证不了「真机上看起来对」**。任务里但凡出现「跑起来看看 / 效果怎么样 / 是不是错位」,**必须上设备截图核验**,不许拿这三层结案。
>
> 一句话:**这三层是用来省设备时间的,不是用来躲开设备的。**
...
| **设备层**(SKILL 主体,**本 skill 的主场**) | 真实渲染、真机交互、系统集成、真实数据、真机专属能力 | 需设备、慢,但**这才是本 skill 存在的理由** |

口诀两句,**缺一不可**:

1. **能离线证明的逻辑,别占真机时间**——纯逻辑 bug 秒级定位,别拿几十分钟真机时间换。
2. **该看 UI 的,一定要真上设备看**——离线层再绿也不算看过 UI。**第 2 句优先级高于第 1 句**:省时间是优化,漏看 UI 是没干活。
...
| 答案 | 归属 |
|---|---|
| 不必须(点了之后状态/文案/路由变没变、禁用态对不对、错误提示出没出) | **本层 ②**(作为回归网长期锁住) |
| 必须(真实渲染观感、原生插件/相机/推送、真实网络与账号、平台差异) | 设备层 |

> ⚠️ **这张表切的是"回归网放哪层",不是"要不要上设备看"**。同一个交互完全可以两边都做:widget test 长期锁住行为不回退,**同时**上设备真点一遍确认真机上的观感与时序。**本次改了 UI,就还是要上设备看一眼**——widget test 通过只说明逻辑没写错,不说明真机上好用。
...
1. **离线全绿 ≠ UI 验过了**(最重要的一条):这三层跑在无头环境,证不了真机渲染观感。改了 UI、或任务说了「跑起来看看」,**必须上设备截图核验**——本 skill 的产出是那张截图,不是 `flutter test` 的绿。
2. **纯逻辑 bug 别占真机时间**:解析/数值/空指针这类离线层秒级定位到行;反复跑的静态视觉回归交给 golden。这条是**效率优化**,优先级低于第 1 条。
3. **`--update-goldens` 不是修复手段**:失败先读 `isolatedDiff` 判断变化是否预期,是才更新基线——无脑更新等于改测试绕过断言。
4. **`meetsGuideline(labeledTapTargetGuideline)` 是元素驱动的前置门禁**:让"控件没包 Semantics"在离线层就挂,别等上了设备才发现点不到。
5. **依赖坏了不崩、不漏内部异常**:外部抛异常 / 超时都收敛成领域内中性状态(`inconclusive`),不外泄内部错、不抛、不当 `failed`(策略 C)。
6. **fixture 只抽断言要用的最小字段子集**,不存全量;抓不到给可操作报错(策略 A)。
7. **字节 fixture 每个 add 注释字节偏移区间**,对着协议表写(策略 B)。
8. **金额 / 大整数用 `BigInt`,不用 `double`**(策略 B)。
9. **probe ≠ 回归**:探查脚本拿到结论写进注释,不留在测试套件里(策略 D)。
10. **离线层先全绿再上设备**;`flutter test` 挂 = 逻辑/契约 bug,不上设备直接修。
