...
## 本项目不可逆 / 红线（停下要人工，不自作主张）

`{{IRREVERSIBLE_REDLINES}}` —— skill 四红线（设备物理掉线 / 真金交易 / 私钥密钥 / 不可逆破坏）之外，**本项目特有**的不可逆动作。命中即停、出报告、等人工：

```
{{IRREVERSIBLE_REDLINES}}    # 如：任何上链/付费/扣费操作；改生产配置；删用户数据；触碰密钥/助记词
```
