...
> 本文承接 keystone(`../SKILL.md`):闭环方法论、环境自举四红线、元素驱动优先、Key+Semantics 双标、验证四层、`--pid-file`+`USR1/USR2`、`kill≠force-stop`、提交规范——都在那里,**这里只讲"把单条闭环放大成多路并行、再放大成无人值守"**。
>
...
2. **整晚跑用 mock / 测试账号,别连真实后端写生产**。验证环境与生产数据**物理隔离**。涉及真金交易 / 私钥密钥 / 不可逆破坏 = keystone 四红线,**任何级别都停下要人工**,无人值守不豁免。
3. **始终要证据,没证据不算完成**。每份报告必须含:**验收对照表(逐条 ✅/❌)+ 关键截图 + 改动文件清单 + commit hash**。这是"不验证不报完成"在规模化下的硬化——你早上是靠**证据**而不是靠"它说做完了"来收割的。卡在第 5 轮的失败也要如实标记,别藏。
...
用 mock/测试数据,绝不连生产、不碰真金/密钥/不可逆操作。
